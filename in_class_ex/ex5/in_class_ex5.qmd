...
(Note that for the global model, you can see the coefficients of each independent variable. But we do not see this for the local geographically weighted model because one model is built for each LGA, and hence there are over 4000+ of such coefficients for each variable).
...
In the following code check, we will use `glm` of R to calibrate a logistic regression model for the water point status. We will exclude fields that are not statistically significant, i.e. *local_population_1km* and *water_point_population*.

```{r}
model_sig <- glm(status ~ distance_to_primary_road+
                       distance_to_secondary_road+
                       distance_to_tertiary_road+
                       distance_to_city+
                       distance_to_town+
...
We can also see that our accuracy is approximately 60%. The sensitivity is higher than the specificity, indicating that our true positive is higher (correctly determined approximately 75% true positive) than the true negative (model correctly determines approximately 41% true negative).

### 9.2 Geographically weighted logistic regression model (with fixed bandwidth)

#### 9.2.1 Converting from sf to sp data frame

We will first convert the data to a SpatialPointsDataFrame data type for compatibility with subsequent packages. In here, we have also excluded fields that are not statistically significant, i.e. *local_population_1km* and *water_point_population*.
...
```{r}
bw.fixed.sig <- bw.ggwr(status ~ distance_to_primary_road+
                           distance_to_secondary_road+
                           distance_to_tertiary_road+
                           distance_to_city+
                           distance_to_town+
...
We obtain a fixed bandwidth of 7133.645 m (projection in Nigeria is in metres), which is approximately 7.1 km.

#### 9.2.3 Building fixed bandwidth model

```{r}
gwlr.fixed.sig <- ggwr.basic(status ~ distance_to_primary_road+
                           distance_to_secondary_road+
                           distance_to_tertiary_road+
                           distance_to_city+
                           distance_to_town+
...
From the results, we can see that the Geographically Weighted Regression model has a lower AIC compared to the Generalised Linear Regression (5077.937 for gwLR vs 6351 for generalised LR). This tells us that Geographically Weighted Regression model has improved explainability.
...
When we compare the overall accuracy is now improved to 75.95% (geographically weighted) compared to the 59.95% that we obtained initially in the non-geographically weighted logistic regression model. In addition, we also noted that specificity increased significantly from 41% to 80%, although a slight decrease in sensitivity from 75% to 71% was observed. Nonetheless, the gwLR model has improved explainability compared to the generalised LR model. This implies we should apply a local strategy (looking at surrounding neighbours) instead of a global strategy to understand the factors for water points being functional or non-functional.

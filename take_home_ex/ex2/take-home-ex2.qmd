...
We will then generate the boxplots for Skater clusters. We will need to use `melt()` from **reshape2** package to convert *cluster_summary*data frame with several measurement columns into a data frame in a canonical format, which has one row for every observed (measured) value. We set the ID of the variables, i.e. `id.vars` argument to the cluster ID, "Skater_CLUSTER" as shown in the following code chunk.
...
![](images/paste-5587D933.png){width="320"}
...
```{r, echo = FALSE}
nga_wp_2 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 2, "Cluster 2", "Others"))
map2 <- tm_shape(nga_wp_2) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_3 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 3, "Cluster 3", "Others"))
map3 <- tm_shape(nga_wp_3) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_4 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 4, "Cluster 4", "Others"))
map4 <- tm_shape(nga_wp_4) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_5 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 5, "Cluster 5", "Others"))
map5 <- tm_shape(nga_wp_5) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_6 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 6, "Cluster 6", "Others"))
map6 <- tm_shape(nga_wp_6) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_7 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 7, "Cluster 7", "Others"))
map7 <- tm_shape(nga_wp_7) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_8 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 8, "Cluster 8", "Others"))
map8 <- tm_shape(nga_wp_8) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 

nga_wp_9 <- nga_wp %>% 
    mutate(ClusterID = if_else(.$ClustG_SPCLUSTER == 9, "Cluster 9", "Others"))
map9 <- tm_shape(nga_wp_9) + 
    tm_fill(col= "ClusterID", palette = c("blue", "#FFFFFF")) + 
  tm_borders(alpha = 0.5) 
```
...
```{r, echo=FALSE}
boxplot2 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 2") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot3 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 3") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot4 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 4") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot5 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 5") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot6 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 6") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot7 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 7") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot8 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 8") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 

boxplot9 <- SPmelted %>% 
    filter(ClustG_SPCLUSTER == "ClustGeo, distance Cluster 9") %>% 
    ggplot(aes(x = value, y = variable)) + 
     geom_boxplot() + theme(axis.text.y = element_text(size = 15)) 
...
```{r echo=FALSE}
clust2 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 2) %>% 
    select(c(1:8))
summary2 <- boxplot(clust2, plot=FALSE)$stats
colnames(summary2) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary2)<-c("Min","Q1","Median","Q3","Max")
summary2 <- t(summary2) %>% 
    round(digits = 2)
summary2 <- kable(summary2, linesep = "") %>% 
    kable_styling(font_size = 10)

clust3 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 3) %>% 
    select(c(1:8))
summary3 <- boxplot(clust3, plot=FALSE)$stats
colnames(summary3) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary3)<-c("Min","Q1","Median","Q3","Max")
summary3 <- t(summary3) %>% 
    round(digits = 2)
summary3 <- kable(summary3, linesep = "") %>% 
    kable_styling(font_size = 10)

clust4 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 4) %>% 
    select(c(1:8))
summary4 <- boxplot(clust4, plot=FALSE)$stats
colnames(summary4) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary4)<-c("Min","Q1","Median","Q3","Max")
summary4 <- t(summary4) %>% 
    round(digits = 2)
summary4 <- kable(summary4, linesep = "") %>% 
    kable_styling(font_size = 10)

clust5 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 5) %>% 
    select(c(1:8))
summary5 <- boxplot(clust5, plot=FALSE)$stats
colnames(summary5) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary5)<-c("Min","Q1","Median","Q3","Max")
summary5 <- t(summary5) %>% 
    round(digits = 2)
summary5 <- kable(summary5, linesep = "") %>% 
    kable_styling(font_size = 10)

clust6 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 6) %>% 
    select(c(1:8))
summary6 <- boxplot(clust6, plot=FALSE)$stats
colnames(summary6) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary6)<-c("Min","Q1","Median","Q3","Max")
summary6 <- t(summary6) %>% 
    round(digits = 2)
summary6 <- kable(summary6, linesep = "") %>% 
    kable_styling(font_size = 10)

clust7 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 7) %>% 
    select(c(1:8))
summary7 <- boxplot(clust7, plot=FALSE)$stats
colnames(summary7) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary7)<-c("Min","Q1","Median","Q3","Max")
summary7 <- t(summary7) %>% 
    round(digits = 2)
summary7 <- kable(summary7, linesep = "") %>% 
    kable_styling(font_size = 10)

clust8 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 8) %>% 
    select(c(1:8))
summary8 <- boxplot(clust8, plot=FALSE)$stats
colnames(summary8) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary8)<-c("Min","Q1","Median","Q3","Max")
summary8 <- t(summary8) %>% 
    round(digits = 2)
summary8 <- kable(summary8, linesep = "") %>% 
    kable_styling(font_size = 10)

clust9 <- cluster %>% 
    filter(.$ClustG_SPCLUSTER == 9) %>% 
    select(c(1:8))
summary9 <- boxplot(clust3, plot=FALSE)$stats
colnames(summary9) <- c("wp_functional", "wp_nonfunctional", 
                        "pct_functional", "pct_nonfunctional", 
                        "pct_lowusage", "pct_rural", "pct_pay",
                        "pct_crucial")
rownames(summary9)<-c("Min","Q1","Median","Q3","Max")
summary9 <- t(summary9) %>% 
    round(digits = 3)
summary9 <- kable(summary9, linesep = "") %>% 
    kable_styling(font_size = 10)
```
...
\[4\] [Geopolitical zones of Nigeria](https://en.wikipedia.org/wiki/Geopolitical_zones_of_Nigeria)
